...
if __name__ == '__main__':
    ...
    # Fit a Support Vector Machine to the training data
    support_vector_machine = svm.SVC()

    print ("Fitting a random forest to labeled training data...")
    machine = support_vector_machine.fit( trainDataVecs, train["sentiment"] )

    # Test & extract results
    result = support_vector_machine.predict( testDataVecs )

    # Write the test results
    output = pd.DataFrame( data={"id":test["id"], "sentiment":result} )
    output.to_csv( "Word2Vec_SVM_Without_Stopwords.csv", index=False, quoting=3 )
    print ("Wrote Word2Vec_SVM_Without_Stopwords.csv")
